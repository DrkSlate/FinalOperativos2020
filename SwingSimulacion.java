...
public class SwingSimulacion extends JPanel {
    private Calle calle_1, calle_2, calle_3, calle_4, calle_5;
	...
	private static List<Carro> carros_cola4 = new ArrayList<Carro>();
        private static List<Carro> carros_cola5 = new ArrayList<Carro>();

	// Constantes
	private static final int ANCHO_CALLE_X = 500;//Esto es para la calle 1 y 3
	private static final int ALTO_CALLE_X = 60; // Alto calle 1 y 3
	...
	private void initUI() {
		...
		calle_4 = new Calle(new Rectangle(calle_3.getRectangle().width, calle_2.getRectangle().height, ANCHO_CALLE_X, ALTO_CALLE_X),1, 0, null, null);
                calle_5 = new Calle(new Rectangle(0, 300, ANCHO_CALLE_X, ALTO_CALLE_X),1, 0, null, null);
        }       
	...
	public void paint(Graphics g) {
		...
		g.setColor(Color.GRAY);
                ...
                g.fillRect(calle_5.getRectangle().x, calle_5.getRectangle().y, calle_1.getRectangle().width,calle_5.getRectangle().height);
	...
	public void agregar_carro_calle_x1() {
        ...
        public void agregar_carro_calle_x5(){
                Carro car_5 = new Carro(new Rectangle(calle_5.getRectangle().x + 0, calle_5.getRectangle().y / 2, ANCHO_CARRO, ALTO_CARRO), 1);
		car_5.setCalle(calle_5);
		carros_cola5.add(car_5);
        }
	...
	public void mover(boolean esta_detenida) {
		...
		for (int i=carros_cola5.size()-1;i>=0;i--) {
			Carro carro = carros_cola5.get(i);
			
			if(carro.getEstado()==1) {
				try {
					Carro carro2 = carros_cola5.get(i-1);
					if(carro2.getRectangle().x > carro.getRectangle().x +carro.getRectangle().width + 10 ) {
						carro.setEstado(1);
						carro.setRectangle(new Rectangle(carro.getRectangle().x + 1, carro.getRectangle().y, carro.getRectangle().height, carro.getRectangle().width));
					}
					...
					if(carro.getRectangle().x > carro.getCalle().getRectangle().width + 25) {
						if(esta_detenida) {
							...
							carro.setEstado(1);
							System.out.println("Se metio a la excepcion la cola 5");
							Carro car = carros_cola5.get(i);
							carros_cola5.remove(i);
							System.out.println(car.toString());
							car.setCalle(calle_2);
							carros_cola2.add(car);
							break;
